...
class WasmPackPlugin {
    ...
    _makeEmpty() {
        const outDirPath = path.join(this.crateDirectory, this.outDir);
        try {
        ...
        const outIndexPath = path.join(outDirPath, this.outName + '.js')
        if (!this.error || (this.error && !fs.existsSync(outIndexPath))) {
